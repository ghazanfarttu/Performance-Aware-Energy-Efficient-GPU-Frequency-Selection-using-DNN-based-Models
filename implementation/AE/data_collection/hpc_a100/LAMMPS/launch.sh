...
do
  ...
  do
    ...
    for app in "${apps[@]}"
      ...
      appCmd="./run_lammps.sh${appParams[$appID]}"	
      echo $appCmd
      ./profile $appCmd
      ...
      if [[ $app == "LAMMPS" ]]
      then
        v=$(sed -n '61p' results/LAMMPS_RUN_OUT)
        tokens=( $v )
        wall_time=${tokens[3]}
        echo $wall_time
        wall_time="${wall_time//[$'\t\r\n ']}"
	wt=`echo $wall_time | awk -F: '{ print ($1 * 3600) + ($2 * 60) + $3 }'`
        printf '%s\n' $c_freq $wt | paste -sd ',' >> results/$arch-dvfs-lammps-perf.csv
      fi
      appID=$((appID+1))
    done #apps
    done #runs
